...
export function createBattleRouter(): Router {
  ...
  router.post("/api/battle/challenge", isAuthenticated, async (req: AuthReq, res) => {
    try {
      ...
      logger.error({ err: error }, "[battle] POST /challenge error:");
      res.status(500).json({ message: "Failed to create match" });
    }
  });

  // GET /api/battle/matches — user's active/recent matches
  router.get("/api/battle/matches", isAuthenticated, async (req: AuthReq, res) => {
    try {
      const userId = req.user.id;
      const matches = await storage.getUserActiveMatches(userId);
      res.json(matches);
    } catch (error) {
      logger.error({ err: error }, "[battle] GET /matches error:");
      res.status(500).json({ message: "Failed to fetch matches" });
    }
  });

  // GET /api/battle/match/:matchId — full match state
  router.get("/api/battle/match/:matchId", isAuthenticated, async (req: AuthReq, res) => {
    try {
      const userId = req.user.id;
      ...
      logger.error({ err: error }, "[battle] GET /match/:matchId error:");
      res.status(500).json({ message: "Failed to fetch match" });
    }
  });

  // POST /api/battle/match/:matchId/place — ship placement
  router.post("/api/battle/match/:matchId/place", isAuthenticated, async (req: AuthReq, res) => {
    try {
      const userId = req.user.id;
      const { matchId } = req.params;
      ...
      logger.error({ err: error }, "[battle] POST /place error:");
      res.status(500).json({ message: "Failed to place ships" });
    }
  });

  // POST /api/battle/match/:matchId/attack — fire at a cell
  router.post("/api/battle/match/:matchId/attack", isAuthenticated, async (req: AuthReq, res) => {
    try {
      const userId = req.user.id;
      const { matchId } = req.params;
      ...
      logger.error({ err: error }, "[battle] POST /attack error:");
      res.status(500).json({ message: "Failed to process attack" });
    }
  });

  // GET /api/battle/match/:matchId/tokens — token balance
  router.get("/api/battle/match/:matchId/tokens", isAuthenticated, async (req: AuthReq, res) => {
    try {
      const userId = req.user.id;
      const { matchId } = req.params;
      ...
      logger.error({ err: error }, "[battle] GET /tokens error:");
      res.status(500).json({ message: "Failed to fetch token balance" });
    }
  });

  // POST /api/battle/match/:matchId/powerup — use a power-up
  router.post("/api/battle/match/:matchId/powerup", isAuthenticated, async (req: AuthReq, res) => {
    try {
      const userId = req.user.id;
      const { matchId } = req.params;
      ...
      logger.error({ err: error }, "[battle] POST /powerup error:");
      res.status(500).json({ message: "Failed to use power-up" });
    }
  });

  // POST /api/battle/matchmake — random opponent from shared squads
  router.post("/api/battle/matchmake", isAuthenticated, async (req: AuthReq, res) => {
    try {
      const userId = req.user.id;
      ...
      logger.error({ err: error }, "[battle] POST /matchmake error:");
      res.status(500).json({ message: "Failed to matchmake" });
    }
  });

  // GET /api/battle/leaderboard/:groupId — season wins leaderboard
  router.get("/api/battle/leaderboard/:groupId", isAuthenticated, async (req: AuthReq, res) => {
    try {
      const userId = req.user.id;
      ...
      logger.error({ err: error }, "[battle] GET /leaderboard error:");
      res.status(500).json({ message: "Failed to fetch leaderboard" });
    }
  });

  // GET /api/battle/stats/:userId — personal battle stats
  router.get("/api/battle/stats/:userId", isAuthenticated, async (req: AuthReq, res) => {
    try {
      ...
      logger.error({ err: error }, "[battle] GET /stats error:");
      res.status(500).json({ message: "Failed to fetch stats" });
    }
  });

  // POST /api/battle/match/:matchId/forfeit — forfeit a match
  router.post("/api/battle/match/:matchId/forfeit", isAuthenticated, async (req: AuthReq, res) => {
    try {
      const userId = req.user.id;
      const { matchId } = req.params;
      ...
      logger.error({ err: error }, "[battle] POST /forfeit error:");
